//! Gateway Protocol types for communication between Agnx and gateway plugins.
//!
//! This crate defines the protocol for gateway plugins to communicate with Agnx.
...
//!
//! - **Commands** (Agnx → Gateway): Instructions from Agnx to the gateway
//! - **Events** (Gateway → Agnx): Notifications from the gateway to Agnx
//!
...
/// Commands sent from Agnx to a gateway.
...
/// Events sent from a gateway to Agnx.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(tag = "type", rename_all = "snake_case")]
