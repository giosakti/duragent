...
use duragent_gateway_telegram::{TelegramConfig, TelegramGateway};
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::from_default_env()
                .add_directive("duragent_gateway_telegram=info".parse().unwrap()),
        )
    ...
    if std::io::stdin().is_terminal() {
        eprintln!("Error: duragent-telegram is designed to run as a subprocess of duragent.");
        eprintln!("It communicates via stdin/stdout and should not be run directly.");
        eprintln!();
        eprintln!("To use Telegram gateway, configure it in your duragent.yaml:");
        eprintln!();
        eprintln!("  gateways:");
        eprintln!("    external:");
        eprintln!("      - name: telegram");
        eprintln!("        command: duragent-telegram");
        eprintln!("        env:");
