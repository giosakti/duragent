...
use agnx_gateway_discord::{DiscordConfig, DiscordGateway};
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::from_default_env()
                .add_directive("agnx_gateway_discord=info".parse().unwrap()),
        )
    ...
    if std::io::stdin().is_terminal() {
        eprintln!("Error: agnx-discord is designed to run as a subprocess of agnx.");
        eprintln!("It communicates via stdin/stdout and should not be run directly.");
        eprintln!();
        eprintln!("To use Discord gateway, configure it in your agnx.yaml:");
        eprintln!();
        eprintln!("  gateways:");
        eprintln!("    external:");
        eprintln!("      - name: discord");
        eprintln!("        command: agnx-discord");
        eprintln!("        env:");
