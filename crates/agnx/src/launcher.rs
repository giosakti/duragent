...
//!
//! Auto-starts a local agnx server process when none is running.
...
/// Spawn the agnx server as a background process.
...
mod tests {
    ...
    fn test_launcher_error_display() {
        let err = LauncherError::StartupTimeout {
            timeout_secs: 10,
            log_path: PathBuf::from("/tmp/agnx-server.log"),
        };
    ...
    async fn test_ensure_server_running_with_explicit_url() {
        ...
        let result = ensure_server_running(LaunchOptions {
            ...
            config_path: Path::new("agnx.yaml"),
            config: &config,
            agents_dir: None,
        })
        .await;
