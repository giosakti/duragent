...
"#
    ...
    fn test_resolve_path_absolute() {
        let config_path = Path::new("/etc/agnx/agnx.yaml");
    ...
    fn test_resolve_path_relative() {
        let config_path = Path::new("/etc/agnx/agnx.yaml");
        let relative_path = Path::new(".agnx/sessions");
        let result = resolve_path(config_path, relative_path);
        assert_eq!(result, PathBuf::from("/etc/agnx/.agnx/sessions"));
    }

    #[test]
    fn test_resolve_path_relative_with_dotslash() {
        let config_path = Path::new("/opt/project/config/agnx.yaml");
    ...
    fn test_resolve_path_config_in_current_dir() {
        let config_path = Path::new("agnx.yaml");
        let relative_path = Path::new(".agnx/agents");
        let result = resolve_path(config_path, relative_path);
        // When config has no parent dir, uses "." which joins to just the relative path
        assert_eq!(result, PathBuf::from(".agnx/agents"));
    }

    #[test]
    fn test_resolve_path_config_with_subdir() {
        let config_path = Path::new("config/agnx.yaml");
        let relative_path = Path::new(".agnx/agents");
        let result = resolve_path(config_path, relative_path);
        assert_eq!(result, PathBuf::from("config/.agnx/agents"));
    }
...
gateways:
  external:
    - name: discord
      command: /usr/local/bin/agnx-discord
      args: ["--verbose"]
      restart: always
...
"#
        ...
        assert_eq!(discord.command, "/usr/local/bin/agnx-discord");
