...
This allows the same agent to run with filesystem (default), PostgreSQL, or S3 — just by changing configuration.
...
To prevent unbounded growth of `events.jsonl`, Duragent applies automatic compaction after each snapshot:
...
| Mode | Behavior |
|------|----------|
| `discard` | Remove events covered by the snapshot (default) |
| `archive` | Move old events to `events.archive.jsonl` before removing |
| `disabled` | No compaction (`events.jsonl` grows unbounded) |

**Compaction process:**
1. Snapshots are triggered every ~50 events, on status changes, and on shutdown
2. After a successful snapshot, events with `seq <= checkpoint_seq` are compacted
3. In `archive` mode, old events are appended to `events.archive.jsonl` first
4. Retained events are written atomically via temp file + rename
5. Compaction failure is non-fatal (logged at debug level)

This prevents storage bloat while maintaining full audit trail when archive mode is used.
...
# =============================================================================
sessions:
  ttl_hours: 168                     # Default: 168 (7 days). 0 disables auto-expiry.
  compaction: discard                # discard (default) | archive | disabled
...
| `sessions.compaction` | enum | `discard` | Event log compaction mode: `discard`, `archive`, `disabled` |
