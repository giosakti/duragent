...
| `sender_overrides` | map | `{}` | Per-sender disposition overrides (sender ID → disposition) |
...
Disposition resolution: check `sender_overrides` for the sender ID first, fall back to `sender_default`.
