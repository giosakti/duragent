...
| `allowlist` | list | `[]` | Sender IDs allowed when policy is `allowlist` |

**`groups` fields:**

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `policy` | string | `open` | `open` (accept all), `disabled` (reject all), or `allowlist` (group IDs only) |
| `allowlist` | list | `[]` | Group/chat IDs allowed when policy is `allowlist` |
| `sender_default` | string | `allow` | Default sender disposition within allowed groups |
| `sender_allowlist` | list | `[]` | Sender IDs explicitly allowed (overrides `sender_default`) |
| `sender_blocklist` | list | `[]` | Sender IDs explicitly blocked (overrides `sender_default` and `sender_allowlist`) |
...
Disposition resolution order for a sender in a group:
1. Check `sender_blocklist` — if matched: `block`
2. Check `sender_allowlist` — if matched: `allow`
