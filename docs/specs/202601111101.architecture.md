...
This document describes Agnx's internal architecture, including the runtime, services, gateways, and sandbox systems.
...
An agent is defined by a YAML file with unstructured content (e.g. prompts, instructions) stored as Markdown files. See [Agnx Agent Format](./202601111200.agnx-agent-format.md) for the full specification.
...
memory:
  ...
  long_term:
    ...
    path: ./.agnx/memory/   # Markdown files
...
Gateways handle communication between users and agents. Agnx separates **core protocols** (built-in) from **platform integrations** (plugins).

### Core Gateways (Built-in)

These are integration protocols that ship with Agnx:

| Gateway | Use Case | Library |
|---------|----------|---------|
| **CLI/TUI** | Local development, `agnx chat`, `agnx attach` | clap + ratatui |
| **HTTP REST** | Programmatic access, webhooks | axum |
| **SSE** | LLM token streaming | axum |

### Platform Gateways (Plugins)

Platform-specific integrations (Telegram, Discord, etc) run as separate processes communicating via the **Gateway Protocol** (JSON over stdio). This keeps Agnx core minimal while allowing any platform integration.
...
```rust
// Agnx → Gateway (stdout of gateway process)
enum ToGateway {
...
// Gateway → Agnx (stdin of gateway process)
enum FromGateway {
...
| Release cycle | Tied to Agnx | Independent |
...
**Plugin Discovery:** Gateway plugins are discovered exclusively via the `gateways.external` configuration in `agnx.yaml`. There is no implicit path scanning or auto-discovery; all plugins must be explicitly configured.
...
Sessions are the core durability primitive in Agnx. A session is a persistent conversation context that survives crashes, restarts, and disconnects.
...
To prevent unbounded growth of `events.jsonl`, Agnx applies automatic compaction:
...
For terminal users, `agnx attach` provides a convenient way to connect to sessions:

```bash
agnx sessions                           # List sessions
agnx chat --agent my-assistant          # Start new session
agnx attach session_abc123              # Reconnect to existing session
```

For persistent terminal sessions, standard tools like tmux or zellij work alongside Agnx.
...
pub trait Sandbox: Send + Sync {
    ...
    /// Pause/hibernate the sandbox (for session persistence)
...
Agnx can orchestrate external coding agents (Claude Code, OpenCode, Aider). A **supervisor agent** running on Agnx delegates tasks to specialized **worker agents** and reviews their output.
...
Agnx provides built-in tools for invoking worker agents:
...
User (async, on phone):
  "Implement user authentication with JWT.
   Review carefully and create a PR when done."
         ...
         ▼
┌─────────────────────────────────────────────────────────────────┐
│  Agnx Supervisor Agent (always running)                         │
│                                                                 │
│  1. Breaks down task:                                           │
│     - Add JWT library                                           │
...
```yaml
# .agnx/sessions/{supervisor_session}/workers.yaml
workers:
...
Agnx supports two agent loading modes:

| Mode | Use Case | Source |
|------|----------|--------|
| **Static** | GitOps, version-controlled agents | Files in `.agnx/agents/` |
| **Dynamic** | API-deployed, multi-tenant | Admin API or dynamic path |

Agnx uses lazy loading with an LRU cache. Server deployments can handle thousands of agents; edge deployments prioritize efficiency for a smaller number of agents.

### Agent Routing

When a message arrives from a gateway, Agnx determines which agent handles it via a **routing table**. Routes are evaluated in order; first match wins.
...
|------|--------------|----------|
| **Built-in** | Bundled with Agnx | Core operations |
...
| Tool | Purpose |
|------|---------|
| `task_list` | View current tasks in a TaskList |
| `task_create` | Create task with optional dependencies |
| `task_update` | Update status, add notes |
| `task_complete` | Mark task as done |
| `claude_code_exec` | Invoke Claude Code in headless mode |
| `opencode_exec` | Invoke OpenCode in headless mode |
| `notify_user` | Send notification (Slack, email, webhook) |
...
For dangerous operations (shell commands, file writes, external API calls), Agnx supports an approval workflow:
...
Agnx provides structured observability from day one.
...
```json
{"ts":"2025-01-24T10:00:00Z","level":"info","target":"agnx::runner","session_id":"abc123","agent":"my-assistant","message":"tool_invoked","tool":"read_file"}
```
...
logging:
  ...
  file: ./.agnx/agnx.log   # Optional file output
```
...
|--------|------|-------------|
| `agnx_sessions_active` | Gauge | Currently active sessions |
| `agnx_messages_total` | Counter | Total messages processed |
| `agnx_tool_calls_total` | Counter | Tool invocations by tool name |
| `agnx_llm_tokens_total` | Counter | Tokens used (input/output) |
| `agnx_llm_latency_seconds` | Histogram | LLM response latency |
...
Agnx agents can expose an [A2A Agent Card](https://a2a-protocol.org/latest/specification/) for discovery:
...
server:
  host: 0.0.0.0
  port: 8080
  request_timeout_seconds: 300     # timeout for non-streaming requests
  idle_timeout_seconds: 60         # close SSE stream if no tokens for this duration
  keep_alive_interval_seconds: 15  # send keep-alive comments to prevent proxy timeouts
...
# Services (state backends)
services:
  session:
    backend: filesystem                # or: postgres, redis
    path: ./.agnx/sessions/
...
# Session defaults (can be overridden per-agent)
session:
  on_disconnect: continue              # or: pause
  max_background_runtime: 30m
  output_buffer_size: 1000

# Sandbox
sandbox:
  backend: auto                        # or: bubblewrap, docker, none
  workspace: ./workspace
  network:
    enabled: false
    allowed_domains:
...
agents:
  ...
  dynamic:
    path: ./.agnx/sys/dyn_agents       # API-deployed agents
  lazy_load: true
  cache_size: 1000                     # LRU cache for scale

# Agent routing (first match wins)
routes:
  - agent: vip-support
    match:
      channel: telegram
  ...
  - agent: eng-bot
    match:
      ...
      workspace: "T12345"
      conversation: "#engineering"
  - agent: default                     # Fallback (no match)
...
spec:
  ...
  session:
    on_disconnect: continue            # or: pause
    max_background_runtime: 30m
    task_list: my-project              # Optional: attach to shared TaskList
