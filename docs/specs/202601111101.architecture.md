...
### agnx.yaml (Current Implementation)
...
# Agent Directory
# =============================================================================
agents_dir: .agnx/agents               # Relative to config file directory
...
services:
  session:
    path: .agnx/sessions               # Relative to config file directory
...
| `agents_dir` | path | `.agnx/agents` | Agent definitions directory |
| `services.session.path` | path | `.agnx/sessions` | Session storage directory |
...
**Path Resolution:** All relative paths (`agents_dir`, `services.session.path`, `gateways.external[].command`) are resolved relative to the config file directory, not the current working directory. This ensures consistent behavior regardless of where `agnx` is run from.
