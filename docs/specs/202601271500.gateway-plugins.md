...
Platform gateway system enabling Duragent to communicate with messaging platforms (Telegram, WhatsApp, Discord). Supports both built-in gateways (compiled in) and external gateways (subprocess).

## The Problem

Duragent core provides CLI, HTTP, and SSE interfaces, but users want to interact with agents via messaging platforms. Each platform has different APIs, authentication flows, and message formats. We need a unified gateway architecture that's simple for self-hosters yet flexible for production deployments.

## User Scenarios

### Scenario 1: Self-host with Telegram

1. User installs `duragent` (default build includes Telegram)
2. User adds `TELEGRAM_BOT_TOKEN` to config
3. User runs `duragent serve` — Telegram bot starts automatically
4. Messages to the bot are routed to configured agent

### Scenario 2: Production with Crash Isolation

1. User builds `duragent --no-default-features` (minimal core)
2. User runs Telegram gateway as separate process
3. Gateway crashes don't affect Duragent core
4. Duragent restarts gateway automatically per restart policy
...
3. User configures external gateway in `duragent.yaml`
4. Duragent spawns and supervises the custom gateway

## Core Features

- **Gateway Protocol**: JSON-over-stdio communication between Duragent and gateways
...
```yaml
# .duragent/sessions/{session_id}/snapshot.yaml
schema_version: 1
...
└── crates/
    ...
    ├── duragent-gateway-protocol/    # Shared protocol types (for external devs)
    │   ├── Cargo.toml
    │   └── src/lib.rs
    └── duragent-gateway-telegram/    # Telegram gateway
...
- [x] Gateway Protocol spec implemented in `duragent-gateway-protocol` crate
...
- [x] Subprocess gateways die when Duragent dies (no orphans via `prctl` + stdin EOF)
