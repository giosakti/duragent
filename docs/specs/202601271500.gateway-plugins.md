...
Gateways support configurable routing rules to direct different chats to different agents based on message context.
...
gateways:
  telegram:
    ...
    default_agent: "general-assistant"  # Fallback when no rule matches
    ...
    routing:
      ...
      - match:
          chat_type: "group"  # Route all groups to moderator
        agent: "group-moderator"
```

### Rules

- **Optional**: Omit the `routing` section for simple single-agent setups
- **Order matters**: Rules are evaluated top-to-bottom, first match wins
- **AND logic**: All conditions in `match` must be satisfied
- **Fallback**: If no rule matches, uses `default_agent`; if that's not set, uses first available agent
