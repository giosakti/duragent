...
Group allowlists use a `{channel}:{chat_id}` composite format (e.g. `telegram:-100123456`, `discord:1234567890`). Allowlists support trailing `*` wildcards (e.g. `telegram:*` for all Telegram groups, `*` for all groups).
...
|------|----------|----------|
| `batch` (default) | Wait for a time window, batch all messages into one request | Rapid message senders |
| `interrupt` | Cancel current processing, restart with new message | Time-sensitive, latest matters |
| `sequential` | Complete current, then process queued one at a time | Preserve all context |
...
Delivered: configurable message queuing with per-session queue management, overflow strategies, and per-sender debouncing.

**Queue modes** (`spec.access.groups.queue.mode`):
- `batch` (default) — after processing completes, combine all pending messages into one and process it
- `sequential` — after processing completes, take the next pending message and process it (repeats until queue is empty)
- `drop` — after processing completes, discard all pending messages

**Overflow strategies** (`spec.access.groups.queue.overflow`):
- `drop_old` (default) — evict oldest pending message to make room
- `drop_new` — reject the new message silently
- `reject` — reject the new message with an optional `reject_message` sent back to the user

**Per-sender debouncing** (`spec.access.groups.queue.debounce`):
- `enabled` (default: true) — batch rapid messages from the same sender
- `window_ms` (default: 1500) — idle window before flushing the debounce buffer
- Fast path: first message from a sender in an idle session bypasses debouncing (zero latency)
- Debounced messages are combined with newlines and flushed into the pending queue

**Implementation details:**
- `SessionMessageQueue` replaces `message_locks` (KeyedLocks) for all message processing (DMs and groups)
- DMs use `QueueConfig::default()` which gives equivalent behavior to the previous mutex
- `message_locks` retained only for callback queries (approval button presses)
- Drain loop runs after each message is processed, sending responses via `gateways.send_message()`
- `SessionMessageQueues` (DashMap) mirrors the `KeyedLocks` pattern with periodic cleanup of idle queues

**Queue config defaults:** `max_pending: 10`, `overflow: drop_old`, `debounce.enabled: true`, `debounce.window_ms: 1500`

### Phase 3.1: Wildcard Patterns in Allowlists ✅

Delivered: trailing `*` wildcard support in all allowlists and sender overrides, composite `{channel}:{chat_id}` format for group allowlists.

**Group allowlist composite format:**
- Entries match against `{channel}:{chat_id}` (e.g. `telegram:-100123456`)
- `telegram:*` matches all Telegram groups, `discord:*` matches all Discord channels
- `*` alone matches all groups on all platforms

**DM allowlist wildcards:**
- Match against bare `sender_id` with trailing `*` support (e.g. `user_*`)

**Sender override wildcards:**
- Exact match takes priority over wildcard match
- Wildcard patterns (e.g. `admin_*`) fall back after exact lookup
- Falls back to `sender_default` if no match

### Phase 4: Advanced Features
- `interrupt` queue mode (cancel current processing, restart with new message)
