...
cron:
  enabled: true
  store: ./.agnx/cron/             # Persistent job state
  max_concurrent: 4
...
```bash
agnx cron list                      # List all jobs
agnx cron add --name "..." --schedule "..." --agent "..."
agnx cron remove <job_name>
agnx cron run <job_name>            # Trigger immediately
agnx cron logs <job_name>           # View run history
```
...
webhooks:
  ...
  token: ${AGNX_WEBHOOK_TOKEN}      # Required for auth
...
curl -X POST http://localhost:8080/hooks/github \
  -H "Authorization: Bearer $AGNX_WEBHOOK_TOKEN" \
...
```yaml
# .agnx/transforms/github-event.yaml
name: github-event
template: |
  GitHub event: {{ action }}
  {% if pull_request %}
...
# Available env vars:
# AGNX_SESSION_ID, AGNX_AGENT_ID, AGNX_EVENT_TYPE
# AGNX_MESSAGE_COUNT, AGNX_LAST_MESSAGE, etc.

echo "Session $AGNX_SESSION_ID ended after $AGNX_MESSAGE_COUNT messages"

# Save summary to memory
curl -X POST "http://localhost:8080/api/v1/memory" \
  -d "{\"agent\": \"$AGNX_AGENT_ID\", \"content\": \"Session ended...\"}"
```

### Hook Locations (Precedence)

1. `<workspace>/hooks/` — Agent-specific hooks
2. `.agnx/hooks/` — Project-level hooks
3. `~/.agnx/hooks/` — User-level hooks
...
Native apps (iOS, Android, macOS) that connect to Agnx for device-level capabilities.
...
Nodes discover Agnx gateway via:
1. mDNS (`_agnx-gw._tcp`)
...
```bash
# On Agnx server
agnx nodes pending               # List pending pairing requests
agnx nodes approve <request_id>  # Approve device
agnx nodes list                  # List connected nodes
agnx nodes revoke <node_id>      # Revoke access
```
...
browser:
  ...
  profiles:
    default:
      user_data_dir: ./.agnx/browser/default
    work:
      user_data_dir: ./.agnx/browser/work
```
...
```bash
agnx skills search "github"
agnx skills install github-pr-review
agnx skills update --all
agnx skills list
```
