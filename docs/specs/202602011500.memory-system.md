...
```
{workspace}/                     # default: .agnx (resolved relative to config file)
├── memory/
...
### Global Config (`agnx.yaml`)

```yaml
workspace: .agnx              # default: .agnx (resolved relative to config file)
...
- If `memory:` section exists in agent config, all four memory tools are registered and a memory directive is added to the system prompt
...
All agents share the same world knowledge directory configured in `agnx.yaml`. Agent-specific memory (`MEMORY.md` and daily logs) lives in `{agent_dir}/memory/`, co-located with the agent's configuration.
...
1. `create_memory_tools(memory)` creates all four tools
